-- Müşteri Ana Tablosu
IF OBJECT_ID('tbl_musteri', 'U') IS NULL
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_tbl_musteri_Unvan' AND object_id = OBJECT_ID('tbl_musteri'))
    CREATE NONCLUSTERED INDEX IX_tbl_musteri_Unvan ON tbl_musteri(Unvan);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_tbl_musteri_MusteriKodu' AND object_id = OBJECT_ID('tbl_musteri'))
    CREATE NONCLUSTERED INDEX IX_tbl_musteri_MusteriKodu ON tbl_musteri(MusteriKodu);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_tbl_musteri_VergiNo' AND object_id = OBJECT_ID('tbl_musteri'))
    CREATE NONCLUSTERED INDEX IX_tbl_musteri_VergiNo ON tbl_musteri(VergiNo);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_tbl_musteri_EkleyenUserId' AND object_id = OBJECT_ID('tbl_musteri'))
    CREATE NONCLUSTERED INDEX IX_tbl_musteri_EkleyenUserId ON tbl_musteri(EkleyenUserId);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_tbl_musteri_Sil' AND object_id = OBJECT_ID('tbl_musteri'))
